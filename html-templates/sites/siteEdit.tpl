...
{if $data->isPhantom}
    <div class="alert alert-danger">Site creation is not yet available, create a <a href="/deployments/create">deployment</a> instead.</div>
{else}
    <div class="row">
        <div class="col-sm-12 col-md-6">
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <select name="DeploymentID" id="DeploymentID" class="form-control" required {if !$data->isPhantom}disabled{/if}>
                        ...
                        {foreach item=Deployment from=$deployments}
                            <option value="{$Deployment->ID}" {if $Deployment->ID == $data->DeploymentID}selected{/if}>{$Deployment->Label}</option>
                        {/foreach}
                    </select>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="UpdateSSL">{_ "Update SSL Config"}: <input type="checkbox" name="UpdateSSL" id="UpdateSSL" /></label>
                </div>
